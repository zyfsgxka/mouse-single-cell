...
P7_1.d <- Read10X(data.dir = ".\\1")
P7_2.d <- Read10X(data.dir = ".\\2")


# -- Create seurat object (non-normalized data)
## --- features: detected in > 3 cells
## --- cells: > 200 features are detected
P7_1<-CreateSeuratObject(count= P7_1.d, project = "P7_1", min.cells = 3, min.features = 200)
P7_2<-CreateSeuratObject(count= P7_2.d , project = "P7_2", min.cells = 3, min.features = 200)

# -- QC checking:
## --- calculate mitochondrial percent:
P7_1[["percent.mt"]] <- PercentageFeatureSet(P7_1, pattern = "^mt-")
P7_2[["percent.mt"]] <- PercentageFeatureSet(P7_2, pattern = "^mt-")
...
P7_merge <- merge(P7_1, y = c(P7_2), project = "P7_merge")
P7_merge # check cells

## --- Visualize QC metrics as a violin plot:
VlnPlot(P7_merge, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
FeatureScatter(P7_merge, feature1 = "nCount_RNA", feature2 = "percent.mt")
FeatureScatter(P7_merge, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
...
Rmax <- 3000
mt.per <- 5
P7_merge <- subset(P7_merge, subset = nFeature_RNA > Rmin & nFeature_RNA < Rmax & percent.mt < mt.per)
P7_merge # check cells
...
s.object <- P7_merge  # define Seurat object
...
P7 <- DimPlot(s.object, reduction = "umap",label = TRUE)
P7
P7_time <- DimPlot(s.object, reduction = "umap",label = TRUE,group.by = "time",label.size = 6)+
theme(legend.text= element_text(size=20), legend.title=element_text(size=15))
P7_time
P7+P7_time
saveRDS(s.object, file = "P7_s.object")
...
P7.cca <- SplitObject(P7_merge, split.by = "time")
#P7.cca <- SplitObject(s.object, split.by = "time")

# -- normalize and identify variable features for each dataset independently:
nG <- 2000
for (i in 1:length(P7.cca)){
  P7.cca[[i]]<- NormalizeData(P7.cca[[i]],verbose = FALSE)
  P7.cca[[i]]<- FindVariableFeatures(P7.cca[[i]],selection.method = "vst",
                                      nfeatures = nG, verbose = FALSE)
}

# -- Perform integration
reference.list <- P7.cca[c("P7_1","P7_2")]
CCA.anchors <- FindIntegrationAnchors(object.list=reference.list,dims = 1:30) 
## --- create an integrated data assay
P7.integrated <-IntegrateData(anchorset=CCA.anchors,dims = 1:30)

### -- analysis for integrated datasets -- ### 
# -- switch to integrated assay.
cca.object <- P7.integrated # define object
...
saveRDS(cca.object, file = "P7_cca_clustered.object")
#cca.object <- readRDS("P7_cca_clustered.object")
...
ref_marker.d <- read.csv("P7_top_gene_paper.txt",sep="\t",header = T)
...
cca.object<-readRDS('P7_cca_clustered.object')
  new.cluster.ids_2 <- c("L.KO","unknown","unknown","M.KO","unknown","unknown","IPhC","CC/OS","HeC/Glia",
                       "OPC/IPC/DC","HC","unknown","L.KO","HC","unknown")
names(new.cluster.ids_2) <- levels(cca.object)
cca.named <- RenameIdents(cca.object, new.cluster.ids_2)
p5<-DimPlot(cca.named, reduction = "umap", label = TRUE, pt.size = 0.5,label.size = 7) +ggtitle("P7_named")+NoLegend()
p4+p5
p5

saveRDS(cca.named, file = "P7.cca.named")
...
DimPlot(P7_cca, reduction = "umap", label = TRUE, pt.size = 0.5,label.size = 7) +ggtitle("P7_named")+NoLegend()
...
p5<-DimPlot(cca.renamed, reduction = "umap", label = TRUE, pt.size = 0.5,label.size = 5) +ggtitle("P7_named")+NoLegend()
p4+p5
p5

saveRDS(cca.renamed,file ="P7_cca.renamed")



##################P7_select_sc_hc########################
setwd("C:\\Users\\zhaoyf\\OneDrive - st.shou.edu.cn\\桌面\\repeat2 mouse\\P7_repeat")
P7_cca.renamed<-readRDS('P7_cca.renamed')
P7_select_SC_HC <- P7_cca.renamed[,P7_cca.renamed@meta.data$seurat_clusters %in% c(6,7,8,9,10,13)]
saveRDS(P7_select_SC_HC , file = "C:\\Users\\zhaoyf\\OneDrive - st.shou.edu.cn\\桌面\\repeat2 mouse\\select_SC_HC-seurat_foldchange\\P7_select_SC_HC")
write.table(P7_select@assays$RNA@counts, "C:\\Users\\zhaoyf\\OneDrive - st.shou.edu.cn\\桌面\\repeat2\\P7_select\\P7_select_matrix.txt",sep="\t", quote=F, col.names=NA)


##################P7_select_only_sc########################
setwd("C:\\Users\\zhaoyf\\OneDrive - st.shou.edu.cn\\桌面\\repeat2 mouse\\P7_repeat")
P7_cca.renamed<-readRDS('P7_cca.renamed')
P7_select_SC <- P7_cca.renamed[,P7_cca.renamed@meta.data$seurat_clusters %in% c(6,7,8,9)]
saveRDS(P7_select_SC , file = "C:\\Users\\zhaoyf\\OneDrive - st.shou.edu.cn\\桌面\\repeat2 mouse\\select only SC\\foldchange\\P7_select_SC")
write.table(P7_select@assays$RNA@counts, "C:\\Users\\zhaoyf\\OneDrive - st.shou.edu.cn\\桌面\\repeat2 mouse\\select only SC\\P7_select_only_SC_matrix.txt",sep="\t", quote=F, col.names=NA)


FeaturePlot(P7_cca.renamed, features = c("Atoh1"),assays=
            pt.size =  0.05, ncol = 1)
...
setwd("C:\\Users\\zhaoyf\\OneDrive - st.shou.edu.cn\\桌面\\repeat2 mouse\\P7_repeat")
P7_cca.renamed<-readRDS('P7_cca.renamed')
P7_select_hc <- P7_cca.renamed[,P7_cca.renamed@meta.data$seurat_clusters %in% c(10,13)]
saveRDS(P7_select_hc , file = "C:\\Users\\zhaoyf\\OneDrive - st.shou.edu.cn\\桌面\\repeat2 mouse\\select only HC\\foldchange\\P7_select_hc")

write.table(P7_select_hc@assays$RNA@counts, "C:\\Users\\zhaoyf\\OneDrive - st.shou.edu.cn\\桌面\\repeat2 mouse\\select only HC\\P7_select_only_HC_matrix.txt",sep="\t", quote=F, col.names=NA)
